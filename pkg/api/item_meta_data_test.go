...
func TestDeleteItemMetaDataSuccess(t *testing.T) {
	...
	err := client.DeleteItemMetaData(1)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
}

func TestGetItemMetaDataSuccess(t *testing.T) {
	server, client := newTestServer(func(w http.ResponseWriter, r *http.Request) {
	...
	result, err := client.GetItemMetaData(nil, nil, nil, intPtr(1))
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if result == nil {
